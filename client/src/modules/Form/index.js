...
const Form = ({
    ...
    const handleSubmit = async(e) => {
        ...
        e.preventDefault()
        const res = await fetch(`https://chat-app-tau-three.vercel.app/${isSignInPage ? 'login' : 'register'}`, {
            method: 'POST',
            headers: {
        ...
        if(res.status === 400) {
            ...
            if(resData.user) navigate('/users/sign_in')
            if(resData.token) {
  ...
  return (
    <div className="bg-light h-screen flex items-center justify-center">
        <div className=" bg-white w-[90%] h-[80%] max-w-[500px] shadow-lg rounded-lg flex flex-col justify-center items-center">
